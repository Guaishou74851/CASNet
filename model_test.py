...
class CASNet(nn.Module):
    ...
    def forward(self, x, q, modes, gamma=0.2822):
        ...
        Q_unfold = batch_correct(Q_unfold, (q - q_basic) * l, self.N - q_basic) + q_basic  # error correction
        ...
        x = PhiT_y
        cs_ratio_map = (Q_unfold / self.N).unsqueeze(2).repeat(1, 1, block_volume).permute(0, 2, 1)  # get expanded CS ratio map R'
